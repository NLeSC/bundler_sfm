...
SIFT::KeyList SIFT::getKeypoints(const char* szImageFile) {
    ...
    if (!pf) {
        std::cerr << "Cannot open " << szImageFile << std::endl;
...
SIFT::KeyList SIFT::getKeypointsFromStdin() {
...
void SIFT::writeKeypoints(const char* szOutputFile, const KeyList& keys) {
    ...
    if (!pf) {
        std::cerr << "Cannot open " << szOutputFile << std::endl;
        return;
    }
...
void SIFT::writeKeypointsToStdout(const KeyList& keys) {
    WriteKeypoints(stdout, keys);
}

void SIFT::writeImage(const char* szOutputFile, const Image& image) {
    FILE* pf = fopen(szOutputFile, "wt");
    if (!pf) {
        std::cerr << "Cannot open " << szOutputFile << std::endl;
        return;
...
int main(int argc, char** argv) {
  ...
  SIFT::PeakThreshInit = value;
  SIFT::KeyList keys = sift.getKeypointsFromStdin();

  sift.writeKeypointsToStdout(keys);
